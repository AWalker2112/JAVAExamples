...
public class Questions {
	...
	public String sandwichFilling(String sandwich) {
		...
		ArrayList<Integer> sandwichCount = new ArrayList();
		...
		for (int i = 0; i < sandwich.length(); i++) {
			...
			if ((sandwich.substring(i, i + bread.length())).equals(bread)) {

				startCount = i;
				sandwichCount.add(i);
				 System.out.println(sandwichCount + "Start Count");

				if (startCount != 0) {

					wordReverse = sandwich.substring(sandwichCount.get(0) + bread.length(), sandwichCount.get(1));
	...
	public String nMid(String input, int n) {
		...
		if (n == 1) {
			...
			output = input.substring(0, ((input.length() / 2 + 1)) - 1)
					+ input.substring((input.length() / 2 + 1), input.length());
			// System.out.println(output);
			return output;
		} else {
			n = ((n - 1) / 2);

			output = input.substring(0, ((input.length() / 2 + 1) - n) - 1)
					+ input.substring((input.length() / 2 + 1) + n, input.length());
			// System.out.println(output);
			return output;
		}
	...
	public int amISearch(String sentence) {
		...
		int counterAmSearch = 0;

		if (sentence.substring(0, 3).equals("am ")) {
			counterAmSearch++;
		}
		
		if (sentence.substring(sentence.length()-3, sentence.length()).equals(" am")) {
			counterAmSearch++;
		}

		for (int i = 0; i < sentence.length() - 3; i++) {

			if (sentence.substring(i, i + 4).equals(" am ")) {
